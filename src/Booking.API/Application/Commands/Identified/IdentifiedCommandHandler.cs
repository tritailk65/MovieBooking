...
namespace BookingService.API.Application.Commands.Identified;
...
public abstract class IdentifiedCommandHandler<T, R> : IRequestHandler<IdentifiedCommand<T, R>, R> where T : IRequest<R>
{
    ...
    public async Task<R> Handle(IdentifiedCommand<T, R> message, CancellationToken cancellationToken)
    {
        ...
        if (alreadyExists)
        {
            // Nếu đã tồn tại trả về kết quả luôn
            return CreateResultForDuplicateRequest();
        }
        else
        {
            // Ghi vào db để đánh dấu
            ...
            try
            {
                ...
                //Swtich case để ghi log một cách minh bạch
                switch (command)
                {
